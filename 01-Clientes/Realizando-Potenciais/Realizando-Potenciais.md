---
type: client
name: "Realizando Potenciais"
status: active
aliases:
  - "RP"
  - "Realizando Potenciais"
---
# Realizando Potenciais

## Contexto

Cliente ativo da Simplifique. Este documento é o índice canônico da estrutura de conhecimento da Realizando Potenciais.

## Estado de clareza

- [[01-Clientes/Realizando-Potenciais/Historico-de-Clareza/EDC-13-RP-2026-08-22|Estado de Clareza — 22/08/2026]]

## Direcionamento e copy — Cúpula da Decisão

> [!note] Comentário de conexão
> Esta estrutura pertence à frente de comunicação e captação da [[01-Clientes/Realizando-Potenciais/Realizando-Potenciais|Realizando Potenciais]]. Ela transforma o direcionamento estratégico da Cúpula em mensagens, peças de conteúdo, campanha de pesquisa e passagem para execução.

- Direcionamento estratégico: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Tese-Marketing-Cupula-Autoridade-Estruturada|Tese de Marketing — Autoridade Estruturada]].
- Arquitetura de mensagens: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Promessas-Cupula-Visao-360|Banco de Promessas 360°]].
- Copy editorial: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Carrosseis-Cupula-12-Territorios|12 carrosséis por território]].
- Captação qualificada: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Carrosseis-Curiosidade-Captacao-Cupula|Carrosséis de curiosidade e captação]].
- Campanha de pesquisa: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Emails-Pesquisa-Cupula-15-Disparos-5-Dias|15 e-mails em cinco dias]].
- Passagem para execução: [[01-Clientes/Realizando-Potenciais/Cúpula da decisão/Briefing-Programacao-Emails-Pesquisa-Cupula|Briefing de programação dos e-mails]].

## Bases canônicas e inteligência operacional

- [[01-Clientes/Realizando-Potenciais/Fontes/Comandinho - ChatRP/RP_BASE_MESTRE_IA_2026-08-17|Base Mestre de IA — 17/08/2026]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Chat Gpt Pablo/RP_RAG_OPERACIONAL_CANONICA_v1.0|RAG Operacional Canônica]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Chat Gpt Pablo/RP_RAG_COMUNICACAO_DOMINIOS_v1.0|RAG de Comunicação e Domínios]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Chat Gpt Pablo/RP_RAG_REUNIOES_LEDGER_v1.0|Ledger de Reuniões]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Drive RP e Compartilhados/Acervo_Master_Assets_RP|Acervo Mestre de Assets]].

## Fontes de conhecimento — Notion

- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/RP-Notion-Import|Importação piloto do Notion]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Base-Padronizada-2.0|Base Padronizada 2.0]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Case-e-Historico-Simplifique|Case e Histórico Simplifique]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Entregas-e-Intervencoes|Entregas e Intervenções]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Gargalos-Identificados|Gargalos Identificados]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Historico-do-Projeto|Histórico do Projeto]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Aprendizados-para-Metodo|Aprendizados para Método]].
- [[01-Clientes/Realizando-Potenciais/Fontes/Notion/Matriz-de-Escopo-e-Finalizacao|Matriz de Escopo e Finalização]].

## Bases operacionais relacionadas

- [[08-Aprendizados/Funis/Indice-de-Funis|Aprendizados — Funis Trabalhados]]
- [[02-Projetos/Projetos|Projetos]]
- [[03-Reunioes/Reunioes|Reuniões]]
- [[04-Decisoes/Decisoes|Decisões]]
- [[09-Metricas/Metricas|Métricas]]
- [[01-Clientes/Clientes|Clientes]]
- [[10-Simplifique/Diretrizes/Simplifique Ops|Simplifique Ops]]
